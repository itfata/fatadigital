...
if (portfolioPreviews.length > 0) {
  ...
  const previewObserver = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        if (entry.isIntersecting) {
          ...
          if (isMobileViewport()) {
            portfolioPreviews.forEach((otherPreview) => {
              if (otherPreview !== preview) {
                unloadPreview(otherPreview);
              }
            });
          }
    ...
    {
      rootMargin: isMobileViewport() ? "80px 0px" : "220px 0px",
      threshold: isMobileViewport() ? 0.3 : 0.08,
    }
  );
